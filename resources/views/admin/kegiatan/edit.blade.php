...
@section('title', 'Edit Kegiatan Sekolah')
@section('breadcrumb', 'Kegiatan Sekolah / Edit')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="kategori" required
                                ...
                                <option value="Academic Event" {{ old('kategori', $kegiatan->kategori) == 'Academic Event' ? 'selected' : '' }}>Academic Event</option>
                                <option value="Extracurricular" {{ old('kategori', $kegiatan->kategori) == 'Extracurricular' ? 'selected' : '' }}>Extracurricular</option>
                                <option value="Public Information" {{ old('kategori', $kegiatan->kategori) == 'Public Information' ? 'selected' : '' }}>Public Information</option>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="time" name="waktu_mulai" value="{{ old('waktu_mulai', $kegiatan->waktu_mulai?->format('H:i')) }}"
                                           class="w-full pl-12 pr-4 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 text-sm transition-all text-slate-700">
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-lg">schedule</span>
                                    <input type="time" name="waktu_selesai" value="{{ old('waktu_selesai', $kegiatan->waktu_selesai?->format('H:i')) }}"
                                           class="w-full pl-12 pr-4 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 text-sm transition-all text-slate-700">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <label id="banner-container" class="border-2 border-dashed border-slate-200 rounded-3xl overflow-hidden text-center bg-slate-50/50 hover:bg-slate-50 transition-all group cursor-pointer block relative {{ $kegiatan->banner_path ? 'aspect-video' : 'p-8' }}">
                                <div id="banner-placeholder" class="{{ $kegiatan->banner_path ? 'hidden' : '' }}">
                                    ...
                                    <h4 class="text-sm font-bold text-slate-700 mb-1">Ganti banner</h4>
                                    <p class="text-xs text-slate-400">Rekomendasi ukuran 1200 x 600 px (Maks. 5MB)</p>
                                </div>
                                <div id="banner-preview" class="{{ $kegiatan->banner_path ? '' : 'hidden' }} absolute inset-0">
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" rows="6"
                                      ...
                                      placeholder="Tuliskan rincian kegiatan sekolah...">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
                        </div>
