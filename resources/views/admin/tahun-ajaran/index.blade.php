...
<div class="flex flex-col gap-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            <a href="{{ route('admin.tahun-ajaran.create') }}" class="flex items-center justify-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl font-bold text-sm hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 w-full sm:w-auto">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.tahun-ajaran.set-active', $ta) }}" method="POST" class="inline">
                                    ...
                                    @method('PUT')
                                    <button type="submit" 
                                            class="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center justify-center border border-emerald-100"
                                ...
                                <a href="{{ route('admin.tahun-ajaran.edit', $ta) }}" 
                                   class="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm flex items-center justify-center border border-indigo-100"
                                   title="Edit Periode">
                                ...
                                <form action="{{ route('admin.tahun-ajaran.destroy', $ta) }}" method="POST" class="inline delete-form">
                                    ...
                                    <button type="button" 
                                            ...
                                            class="w-10 h-10 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all shadow-sm flex items-center justify-center border border-rose-100"
                                            title="Hapus Periode">
